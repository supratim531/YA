...
function ServiceCards({ services }) {
  ...
  let learnMore = (title, longDescription) => {
    setTitle(title);
  ...
  return (
    <>
      <ServiceModal longDescription={longDescription} title={title} close={context.hideServiceModal} />
          ...
          services.map(e => {
            return (
              <div className="px-6 py-16 w-[360px] flex flex-col items-center gap-10 rounded-[50px] duration-200 cursor-pointer hover:-translate-y-2 outline outline-1 outline-slate-400 hover:border-b-4 hover:border-secondary hover:shadow-[0px_0px_12px_4px_rgba(0,0,0,0.3)] hover:shadow-slate-400">
                <div className="">
                  <img className="w-60" src={e.logo} alt="" />
                ...
                <div onClick={() => learnMore(e.title, e.longDescription)} className="font-bold text-center text-primary">Learn More</div>
              </div>
            );
          })
        }
